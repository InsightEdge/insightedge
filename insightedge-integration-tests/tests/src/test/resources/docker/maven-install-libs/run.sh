...
if [ "$#" -ne 3 ]; then
    ...
    echo "Usage: run.sh <dir with InsightEdge zip> <distribution version> <Git branch>"
    exit 1
fi
...
docker exec --user ie-user maven-install-libs-test-image /home/ie-user/maven-install-libs-test.sh $BRANCH
