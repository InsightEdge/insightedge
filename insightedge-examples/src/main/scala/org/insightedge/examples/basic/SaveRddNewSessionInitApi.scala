...
/**
  ...
  */
object SaveRddNewSessionInitApi {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val spark = SparkSession.builder
      .appName("example-save-rdd-new-session-init-api")
      .master(master)
      .getOrCreate()

    //initializing the insightedge context via the spark session
    spark.initializeInsightEdgeContext(config)
