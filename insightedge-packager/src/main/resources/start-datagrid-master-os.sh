...
main() {
    ...
    export GSA_JAVA_OPTIONS="$GSA_JAVA_OPTIONS -Dinsightedge.marker=master_lus"
    export LOOKUPLOCATORS=$GRID_LOCATOR
    export LOOKUPGROUPS=$GRID_GROUP
    export NIC_ADDR=$CLUSTER_MASTER
    #nohup $IE_PATH/datagrid/bin/puInstance.sh $IE_PATH/datagrid/deploy/templates/insightedge-lus > $log 2>&1 &
